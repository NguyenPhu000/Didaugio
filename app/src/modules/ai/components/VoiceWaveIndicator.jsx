...
export function VoiceWaveIndicator({
  ...
  const pulseStyle = useAnimatedStyle(() => {
    const lift = active ? Math.max(0.25, Math.min(1, intensity)) : 0;
    const peakScale = 1.01 + lift * 0.075;
    const scale = interpolate(progress.value, [0, 0.5, 1], [0.95, peakScale, 0.95]);
    return {
      transform: [{ scale }],
      opacity: active ? 0.88 + lift * 0.12 : 0.82,
    };
  });
  ...
  const rotateA = useAnimatedStyle(() => ({
    transform: [
      ...
      { scaleX: interpolate(progress.value, [0, 0.5, 1], [1.03, active ? 1.11 + Math.min(1, intensity) * 0.06 : 1.07, 1.03]) },
      { scaleY: interpolate(progress.value, [0, 0.5, 1], [0.96, active ? 1.03 + Math.min(1, intensity) * 0.05 : 1.01, 0.96]) },
    ],
  }));
  ...
  const glowStyle = useAnimatedStyle(() => ({
    opacity: interpolate(shimmer.value, [0, 1], [0.42, active ? 0.74 + Math.min(1, intensity) * 0.22 : 0.64]),
    transform: [
      { scale: interpolate(shimmer.value, [0, 1], [0.86, active ? 1.02 + Math.min(1, intensity) * 0.18 : 0.98]) },
    ],
  }));
