import { LogOut, Menu, X, LayoutDashboard, MapPin, Users, Settings, Mail, Key, FileText, Monitor } from "lucide-react";
...
const Navbar = () => {
  ...
  return (
    <nav className="bg-slate-950 text-white border-b border-white/10 sticky top-0 z-50 shadow-md">
      <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-12">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <Link to="/dashboard" className="flex items-center gap-3 group">
            <div className="w-8 h-8 bg-white/10 border border-white/15 flex items-center justify-center text-white font-bold rounded-xl">
              D
            </div>
            <div className="flex flex-col">
              <span className="text-base font-bold tracking-tight leading-none text-white transition-colors">
                {t("common.appName")}
              </span>
              <span className="text-[10px] text-slate-400 font-medium leading-none uppercase mt-0.5">
          ...
          <div className="hidden md:flex items-center space-x-1">
            {menuItems.map((item) => (
              <Link
                ...
                to={item.path}
                className={cn(
                  "relative flex items-center px-3.5 py-2 rounded-xl text-xs font-semibold tracking-wide transition-all duration-200 group overflow-hidden border border-transparent",
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent
                className="w-56 bg-slate-900 border-white/10 text-white rounded-xl"
                align="end"
                forceMount
              >
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    ...
                    <p className="text-xs leading-none text-slate-400 font-medium mt-1">
                      {roleLabel}
                    </p>
                  </div>
                </DropdownMenuLabel>
                ...
                <DropdownMenuItem
                  className="focus:bg-white/10 focus:text-white cursor-pointer text-slate-300 text-xs font-medium"
                ...
                <DropdownMenuItem
                  className="focus:bg-red-900/20 focus:text-red-400 text-red-400 cursor-pointer text-xs font-medium"
              ...
              <div className="border-t border-white/10 pt-3 mt-3">
                <div className="flex items-center justify-between px-3">
                  ...
                  <Button
                    ...
                    disabled={isLoggingOut}
                    onClick={handleLogout}
                    className="border-white/20 bg-transparent text-white hover:bg-white/10 hover:text-red-400 rounded-xl"
                  >
                    <AnimatedIcon
                      icon={LogOut}
                      className="h-4 w-4"
